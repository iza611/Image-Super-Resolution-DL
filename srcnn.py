...
def modcrop(image, scale=3):
    """
    To scale down and up the original image, no remainder while scaling operation.

    Find modulo of height (and width) and scale factor.
...
"""
## ------ Weight of three conv layers set to
...
##------ Save the LR and SR images and compute the psnr

SR_image = output_[0][0].numpy()
iio.imsave("./results/HR.png", HR_image) # GT
iio.imsave("./results/LR.png", LR_image) # HR-base
iio.imsave("./results/SR.png", SR_image) # HR-SRCNN

LR_PSNR = peak_signal_noise_ratio(HR_image, LR_image)
SR_PSNR = peak_signal_noise_ratio(HR_image, SR_image)
print(f"LR_PSNR: {LR_PSNR}")
print(f"SR_PSNR: {SR_PSNR}")